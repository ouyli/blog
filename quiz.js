(function($) {
    ...
    var aboutmy=document.getElementById('about').value;
        ...
        var defaults = {
            ...
            resultComments: {
                perfect: '你才高八斗!（看霉霉手势）',
                excellent: '你学富五车!（看霉霉手势）',
                good: '进，可考研；退，可搬砖。（看霉霉手势）',
                average: '你不是雨伞别撑着,哭',
                bad: '你若安好，便是晴天。轰隆！',
                poor: '你之所以选C，是因为它一笔可以写完。',
                worst: '你的优点：每天能交新朋友，做题从不重复。'
            }
        };
       ...
       if (aboutmy==="官网") {
        ...
        answers = [],      
        introFob = '	<div class="intro-container slide-container"><a class="nav-start" href="#">共' + config.questions.length + '题，本次抽选' + ct + '题。点击图片开始。<br/><br/><span><img src="images/'+ Math.floor(Math.random()*6) +'.gif"  width=50%></span></a></div>	',
...
//抽题存入	randoms数组	共i题 randoms[i]即为questionsIteratorIndex序号！	
        ...
        for (i = 0; i < ct; i++) {
			questionsIteratorIndex = randoms[i]
			 if (config.questions[questionsIteratorIndex].ans.length==1){if (config.questions[questionsIteratorIndex].answers.length==2){ tx='判断题：'}else{tx='单选题：'}}else{tx='多选题：'}
            contentFob += '<div class="slide-container"><div class="question-number">' + (i + 1) + '/'+ct+'</div><div class="question">【'+ config.questions[questionsIteratorIndex].km+'】' + config.questions[questionsIteratorIndex].title + '</div><ul class="answers"><div class="correct1"><P>'+ tx+'<P></div>';
            ...
            contentFob += '</ul><div class="nav-container">';		 
                    if(document.getElementById('answ').checked==true){
						...
						contentFob += '<div  class="correct1"><p>参考答案：' + daan +'</p></div><br>';}
        ...
        function judgeSkills(score) {
            ...
            else if (score > 80) return config.resultComments.good;
            else if (score > 60) return config.resultComments.average;
            else if (score > 40) return config.resultComments.bad;
        ...
        superContainer.find('.final').click(function() {
          ...
          //  if(score>79){tttt=15;ttt = trueCount * 2500}else{if(score>59){tttt=16;ttt = trueCount * 2200}else{if(score>39){tttt=18;ttt = 8000}else{tttt=17;ttt = 8000}}}
            resultSet = '<h2 class="qTitle">您的分数：' + score +  '<br/>  ' + judgeSkills(score) + '</h2>' + shareButton + '<div class="correct1"><a class="nav-show-result" href="javascript:again();">再来一次</a></div><div id="mmlove" style="display:none;"><img src="images/'+ tttt +'.gif"  width=50%></div><br/><hr/><div class="jquizzy-clear">' + resultSet + '</div><div class="jquizzy-clear"></div>';
